...
typedef struct Graf {
    ...
    int nr_noduri;
    int nr_depozite;
    int *depozite; // vectorul care contine depozitele
...
void inputs(Graf *g, int n, int m, int d) {
    ...
    for(i = 0; i < m; i++) {
        scanf("%d %d %f", &u, &v, &w); // muchie de la u la v cu cost w
        g->M[u][v] = w;
    }
...
void Tarjan(Graf *g, int nod, int *index, int *id, int *lowLink, int *visited, Stack *s, int *scc, int **mat_scc) {
    ...
    int i, j;
    int *este_depozit = malloc(g->nr_noduri * sizeof(int));
    for(j = 0; j < g->nr_noduri; j++)
        este_depozit[j] = 0;
    for(j = 0; j < g->nr_depozite; j++)
        este_depozit[g->depozite[j]] = 1;
    for(i = 0; i < g->nr_noduri; i++) {
    ...
    if(lowLink[nod] == id[nod] && nod != 0) {
        ...
        while(s->head->val != nod) {
            int nod_scc = pop(s); // eliberez stiva (nod(uri) care formeaza scc)
            visited[nod_scc] == 0;
            crescator[x++] = nod_scc;
        }
        int nod_scc = pop(s);
        visited[nod_scc] == 0;
        crescator[x++] = nod_scc;
        sortArr(crescator, x); // sortez componentele dupa index
        int y;
...
int main() {
    ...
    for(i = 0; i < nr_taskuri; i++) {
        ...
        if(strcmp(task, "e2") == 0) {
            ...
            free(lowLink);
            free(id);
            // eliberam stiva (ce a mai ramas din ea, adica nodurile depozite)
            while(s->head != NULL) {
                Node *aux = s->head;
                s->head = s->head->next;
                free(aux);
